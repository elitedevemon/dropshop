      ...
      <div class="card-header row gutters-5">
        ...
        <div class="col-md-3 ml-auto">
          <select class="form-control aiz-selectpicker" name="payment_status"
            data-placeholder="{{ translate('Filter by Payment Status') }}" onchange="sort_orders()">
                  ...
                  <td class="text-right">
                    @if (addon_is_activated('pos_system') && $order->order_from == 'pos')
                      <a class="btn btn-soft-success btn-icon btn-circle btn-sm"
                        ...
                        title="{{ translate('Thermal Printer') }}" target="_blank">
                        <i class="las la-print"></i>
                      </a>
                    @endif
                    <a class="btn btn-soft-info btn-icon btn-circle btn-sm"
                      href="{{ route('seller.orders.show', encrypt($order->id)) }}"
                      title="{{ translate('Order Details') }}">
                      <i class="las la-eye"></i>
                    </a>
                    <a class="btn btn-soft-warning btn-icon btn-circle btn-sm"
                      href="{{ route('seller.invoice.download', $order->id) }}"
