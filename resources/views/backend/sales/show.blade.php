      ...
      <div class="row gutters-5">
        ...
        @if ($order->seller_id == $admin_user_id || get_setting('product_manage_by_admin') == 1)
          ...
          @if (addon_is_activated('delivery_boy'))
            <div class="col-md-3 ml-auto">
              ...
              @if (
                ...
                <select class="form-control aiz-selectpicker" id="assign_deliver_boy" data-live-search="true"
              ...
              @else
                <input class="form-control" type="text" value="{{ optional($order->delivery_boy)->name }}"
                  disabled>
              @endif
            </div>
          @endif
          ...
          <div class="col-md-3 ml-auto">
            ...
            @else
              <input class="form-control" type="text" value="{{ $payment_status }}" disabled>
            @endif
          </div>
          <div class="col-md-3 ml-auto">
            ...
            @else
              <input class="form-control" type="text" value="{{ $delivery_status }}" disabled>
            @endif
          </div>
          {{-- <div class="col-md-3 ml-auto">
...
?>
          ...
          <table class="table-bordered aiz-table invoice-summary table">
            <thead>
              <tr class="bg-trans-dark">
                <th class="min-col" data-breakpoints="lg">#</th>
                <th width="10%">{{ translate('Photo') }}</th>
                <th class="text-uppercase">{{ translate('Description') }}</th>
                <th class="text-uppercase" data-breakpoints="lg">{{ translate('Delivery Type') }}</th>
                <th class="min-col text-uppercase text-center" data-breakpoints="lg">
                  {{ translate('Qty') }}
                </th>
                <th class="min-col text-uppercase text-center" data-breakpoints="lg">
                  {{ translate('Price') }}</th>
                <th class="min-col text-uppercase text-right" data-breakpoints="lg">
                  {{ translate('Total') }}</th>
              </tr>
            </thead>
            <tbody>
              @foreach ($order->orderDetails as $key => $orderDetail)
                <tr>
                  ...
                  <td>
                    @if ($orderDetail->product != null && $orderDetail->product->auction_product == 0)
                      <a href="{{ route('product', $orderDetail->product->slug) }}" target="_blank">
                        <img src="{{ uploaded_asset($orderDetail->product->thumbnail_img) }}" height="50">
                      </a>
                  ...
                  <td>
                    @if ($orderDetail->product != null && $orderDetail->product->auction_product == 0)
                      <strong>
                        <a class="text-muted" href="{{ route('product', $orderDetail->product->slug) }}"
                          target="_blank">
                    ...
                    @elseif ($orderDetail->product != null && $orderDetail->product->auction_product == 1)
                      <strong>
                        <a class="text-muted"
                          href="{{ route('auction-product', $orderDetail->product->slug) }}" target="_blank">
                          {{ $orderDetail->product->getTranslation('name') }}
                        </a>
                      </strong>
                    @else
      ...
      <div class="clearfix float-right">
        ...
        <div class="no-print text-right">
          <a class="btn btn-icon btn-light" type="button"
            href="{{ route('invoice.download', $order->id) }}"><i class="las la-print"></i></a>
        </div>
      </div>
