...
class OrderController extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    foreach ($seller_products as $seller_product) {
      ...
      $order->code = str_pad(random_int(0, 99999999), 8, '0', STR_PAD_LEFT);
  ...
  public function update_payment_status(Request $request)
  {
    ...
    if (Auth::user()->user_type == 'seller') {
      foreach ($order->orderDetails->where('seller_id', Auth::user()->id) as $key => $orderDetail) {
        ...
        // add commission
        $exist_commission_history = CommissionHistory::where('order_id', $order->id)->first();
        $ordered_user = User::findOrFail($order->user_id);
        if ($request->status == 'paid' && !$exist_commission_history) {
          ...
          $commission_history->admin_commission = $orderDetail->product->commission;
          $commission_history->seller_earning = $orderDetail->product->commission;
          $commission_history->save();

          $user_balance = $ordered_user->shop->admin_to_pay + $orderDetail->product->commission;
          $ordered_user->shop()->update([
            'admin_to_pay' => $user_balance
          ]);
        }
      }
    } else {
